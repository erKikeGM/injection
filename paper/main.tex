...
The evaluation harness should use a mixed benchmark suite. HarmBench provides a standardized framework for automated red teaming and robust refusal evaluation \citep{mazeika2024harmbench}. JailbreakBench provides an open robustness benchmark with adversarial artifacts, a 100-behavior dataset, scoring functions, and a leaderboard \citep{chao2024jailbreakbench}. RealToxicityPrompts provides 100,000 naturally occurring prompts paired with toxicity scores \citep{gehman2020realtoxicity}. TruthfulQA contains 817 questions across 38 categories to test whether models imitate human falsehoods \citep{lin2021truthfulqa}. BBQ evaluates social bias in question answering across protected-class dimensions \citep{parrish2021bbq}.
...
The first experiment should avoid creating a harmful model. It should use existing benchmark prompts, a sandboxed open-source instruction model, no external action authority, sanitized logging, and controlled evaluation. A deployment in medicine, law, finance, aviation, law enforcement, child safety, or public services would require stricter thresholds and domain review before live use.

\begin{enumerate}
  \item Select a small open-source instruction model in a sandbox.
  \item Establish a baseline across HarmBench, JailbreakBench, RealToxicityPrompts, TruthfulQA, BBQ, custom domain prompts, over-refusal prompts, multilingual jailbreak prompts, and tool-abuse prompts.
  \item Apply condition A: prompt-only ethical augmentation.
  \item Apply condition B: prompt augmentation plus input classifier.
  \item Apply condition C: prompt augmentation plus input classifier plus output evaluator.
  \item Apply condition D: full immune layer with classifier, overlay, evaluator, tool gate, audit logging, and escalation simulation.
